...
Describe 'Upload Files' category:"Object Management" id:"054"
  Parameters:matrix
    $PROFILES
    $CLIENTS
  End
  Example "on profile $1, using client $2, upload $local_file to bucket $BUCKET_NAME"
    profile=$1
    client=$2
    key="test-053__${profile}__${client}__${filename}"

    echo "($client) uploading $local_file to $BUCKET_NAME/$key..."
    case "$client" in
    "aws-s3api" | "aws")
