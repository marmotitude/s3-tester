...
Describe 'Benchmark test:' category:"Bucket Management"
  ...
  Example "on profile $1 using client $2" id:"100"
    ...
    aws s3 --profile br-se1 cp ./report/$(date "+%Y-%m-%d.%H")h-dashboard.html s3://benchmark/dashboards/${date}h-dashboard.html --acl public-read > /dev/null
    aws s3 --profile br-se1 cp ./report/$(date "+%Y-%m-%d.%H")h-dashboard.html s3://benchmark/dashboards/index.html --acl public-read > /dev/null
